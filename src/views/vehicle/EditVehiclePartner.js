...
const EditVehiclePartner = () => {
  ...
  const goBack = () => {
    ...
    navigate('/vehicle/plisting');// This will navigate back to the previous page
  };
...
async function getVehicle(id)
  ...
  const handlesubmit = ()=>{
    ...
    if(!VehicleCatgory || !VehicleType || !VehicleCondition || !VehicleShape || !VehicleTitle || !VehicleColor || !VehicleModal
      || !VehiclePower)
      {
        seterror(true);
              ...
              <CForm>
                ...
                <CFormLabel htmlFor="ssasasa">Vehicles Type <span className='mnd'>*</span></CFormLabel>
                  <CFormSelect aria-label="Default select example" onChange={(e)=>SetVehicleType(e.target.value)}>
                    <option selected="">Select Type Vehicles  </option>
                    {
...
export default EditVehiclePartner

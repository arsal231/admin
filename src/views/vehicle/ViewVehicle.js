...
{
  let responseData = await fetch(API_URL+'/get_vehicles');
  ...
  const column = [
      ...
      //  {
      //   name:<b>Request</b>,
      //   selector: (row)=> (row.vehicle_request==0 ? <CBadge color="info">New Request</CBadge> : <CBadge color="success">Approved</CBadge>)
      //  },
       {
        name:<b>Action</b>,
        cell: (row)=><><Link to={"/vehicle/"+row.id} className='btn btn-success me-1'>
